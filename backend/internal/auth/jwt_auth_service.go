...
import (
	...
	"ElainaBlog/internal/util/timeutil"
	"context"
	"crypto/rand"
	"encoding/hex"
	"errors"
...
type JwtAuthService struct {
	...
	Issuer             string
	Redis              *redis.Client // 可选，用于 token 黑名单
}
...
func BlacklistToken(redis *redis.Client, jti string, ttl time.Duration) error {
	if redis == nil || jti == "" {
		return nil
	}
...
func IsTokenBlacklisted(redis *redis.Client, jti string) bool {
	if redis == nil || jti == "" {
		return false
	}
...
func NewJwtAuthServiceWithConfig() *JwtAuthService {
	accessTTL, err := timeutil.ParseDuration(config.GlobalConfig.Auth.AccessTokenExpiryTime)
	if err != nil {
		log.Fatalf("解析 AccessTokenExpiryTime 失败: %v", err)
	}
	refreshTTL, err := timeutil.ParseDuration(config.GlobalConfig.Auth.RefreshTokenExpiryTime)
	if err != nil {
