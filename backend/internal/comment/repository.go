...
func (r *MySQLRepository) GetCommentByID(id int64) (*Comment, error) {
	var comment Comment
	err := r.db.QueryRow(`SELECT id, article_id, user_id, reply_to_user_id, reply_to_username, reply_to_comment_id, reply_to_content, content, created_at
    FROM comment WHERE id = ? AND is_deleted = 0`, id).Scan(&comment.ID, &comment.ArticleID, &comment.UserID, &comment.ReplyToUserID, &comment.ReplyToUsername, &comment.ReplyToCommentID, &comment.ReplyToContent, &comment.Content, &comment.CreatedAt)
	if err != nil {
		return nil, err
	}
...
func (r *MySQLRepository) GetCommentListByArticleID(articleID int64) ([]*CommentVO, error) {
	rows, err := r.db.Query(`
    SELECT c.id, c.article_id, c.user_id, c.reply_to_user_id, c.reply_to_username, c.reply_to_comment_id, c.reply_to_content,
	...
	for rows.Next() {
		var vo CommentVO
		err := rows.Scan(&vo.ID, &vo.ArticleID, &vo.UserID, &vo.ReplyToUserID, &vo.ReplyToUsername, &vo.ReplyToCommentID, &vo.ReplyToContent,
...
func (r *MySQLRepository) CreateComment(comment *Comment) (int64, error) {
	result, err := r.db.Exec(`INSERT INTO comment (article_id, user_id, reply_to_user_id, reply_to_username, reply_to_comment_id, reply_to_content, content) VALUES (?, ?, ?, ?, ?, ?, ?)`,
		comment.ArticleID, comment.UserID, comment.ReplyToUserID, comment.ReplyToUsername, comment.ReplyToCommentID, comment.ReplyToContent, comment.Content)
	if err != nil {
		return 0, err
	}
