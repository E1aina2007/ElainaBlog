...
func SetVerificationCode(client *redis.Client, email, code string, expiry, interval time.Duration) error {
...
func GetVerificationCode(client *redis.Client, email string) (string, error) {
	return client.Get(context.Background(), codeKey(email)).Result()
}

// DeleteVerificationCode 验证成功后删除验证码
func DeleteVerificationCode(client *redis.Client, email string) error {
	return client.Del(context.Background(), codeKey(email)).Err()
}

// IsDuringInterval 检查是否在重发间隔内（防刷）
func IsDuringInterval(client *redis.Client, email string) (bool, error) {
